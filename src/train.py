...
# @hydra.main(config_path='../configs', config_name='config.yaml', version_base='1.3')  # Hydra decorator
def train(cfg: DictConfig) -> Optional[float]:
    ...
    for fold in range(start_fold, end_fold):  # iterate over folds
        ...
        datamodule_train: LightningDataModule = hydra.utils.instantiate(cfg.datamodule,
                                                                        fold=fold)  # instantiate datamodule
        ...
        if "callbacks" in cfg:
            ...
            callbacks[
                0].FILE_EXTENSION = f'_fold-{fold + 1}.ckpt'  # naming of logs for cross validation callbacks[0] is the model checkpoint callback (this is a hacky way to do this)
        ...
        if cfg.get("test_after_training"):  # and not 'simclr' in  cfg.model._target_.lower():
            ...
            for set in cfg.datamodule.cfg.testsets:
                ...
                log_dict = utils.summarize(preds_dict['val'][set],
                                           'val')  # sets prefix val/ and removes lists for better logging in wandb
                ...
                log_dict.update(utils.summarize(preds_dict['test'][set],
                                                'test'))  # sets prefix test/ and removes lists for better logging in wandb
